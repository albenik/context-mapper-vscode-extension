...
export async function askForName(prompt: string, initialValue: string): Promise<string | undefined> {
...
export async function askForVerb(prompt: string, initialValue: string): Promise<string | undefined> {
    const inputBoxOptions: InputBoxOptions = {
        value: initialValue,
        prompt: prompt,
        validateInput: (value: string) => {
...
export async function askForStringSelection(prompt: string, values: string[]): Promise<string | undefined> {
    return await window.showQuickPick(values, { placeHolder: prompt });
}

export async function askForMultipleStringSelection(prompt: string, values: string[]): Promise<string[] | undefined> {
